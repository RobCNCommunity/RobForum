...
<template>
  <PageContainer title="收藏">
    ...
    <section v-else class="rf-timeline rf-bookmark-timeline"><RouterLink v-for="post in posts" :key="post.id" :to="`/posts/${post.id}`" class="rf-post-row"><UserAvatar :src="post.author_avatar" :name="post.author_name" :size="42" /><div class="rf-post-body"><div class="rf-post-meta"><strong>{{ post.author_name }}</strong><VerifiedBadge :verified="post.author_verified" /><MembershipBadge :active="post.author_member" :tier-id="post.author_membership_tier_id" /><span>·</span><time>{{ new Date(post.created_at).toLocaleDateString('zh-CN') }}</time></div><PostTagList :tags="post.tags" compact /><h2 v-if="post.title">{{ post.title }}</h2><p :class="{ 'rf-post-primary-copy': !post.title }">{{ excerpt(post.content) }}</p><PostMediaGrid v-if="post.media?.length" :media="post.media" compact :preview="false" /><div class="rf-post-actions"><span><AppIcon name="message" size="17" />{{ post.comment_count }}</span><span><AppIcon name="repost" size="17" />{{ post.repost_count || 0 }}</span><span><AppIcon name="heart" size="17" />{{ post.like_count || 0 }}</span></div></div></RouterLink></section>
  </PageContainer>
</template>
