...
async function load() {
  ...
  try {
    ...
    error.value = errorMessage(cause, '社区公告加载失败')
  } finally {
    loading.value = false
  }
}
...
<template>
  <section class="rf-page news-page">
    <header class="rf-page-header news-header">
      <div class="rf-page-heading"><p class="news-kicker">COMMUNITY ANNOUNCEMENTS</p><h1>社区公告</h1><p>论坛维护、规则调整和活动消息会集中发布在这里。</p></div>
    ...
    <div v-else-if="!notices.length" class="rf-empty"><AppIcon name="announcement" size="30" /><strong>暂无社区公告</strong><span>新的公告发布后会显示在这里。</span></div>
