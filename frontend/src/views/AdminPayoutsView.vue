...
<template>
  <PageContainer title="创作者提现">
    ...
    <div v-else class="rf-data-table" role="table" aria-label="创作者提现列表">
      <div class="rf-data-row rf-data-head" role="row"><span>创作者</span><span>金额</span><span>收款方式</span><span>状态</span><span>申请时间</span><span>操作</span></div>
      <article v-for="item in items" :key="item.id" class="rf-data-row" role="row">
        ...
        <div data-label="金额"><b class="rf-money">{{ formatMoney(item.amount_cents) }}</b></div>
    ...
    <div v-if="modalOpen" class="rf-modal-backdrop rf-modal-backdrop--center" @click.self="closeReview">
      <section class="rf-dialog" role="dialog" aria-modal="true" :aria-label="review.status === 'paid' ? '确认已完成打款' : '驳回提现申请'">
        ...
        <div v-if="review.item" class="rf-review-summary"><span>{{ review.item.creator_name || `用户 #${review.item.creator_id}` }}</span><strong>{{ formatMoney(review.item.amount_cents) }}</strong></div>
