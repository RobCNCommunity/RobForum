...
function reset() { Object.assign(form, { id: 0, title: '', content: '', link_url: '', level: 'info', pinned: false, enabled: true }) }
function edit(item: Notice) { Object.assign(form, { id: item.id, title: item.title, content: item.content, link_url: item.link_url || '', level: item.level || 'info', pinned: item.pinned, enabled: item.enabled }) }
...
async function save() {
  ...
  saving.value = true
  try {
    const payload = { title: form.title.trim(), content: form.content.trim(), link_url: form.link_url.trim(), level: form.level, pinned: form.pinned, enabled: form.enabled }
    ...
    <div class="admin-grid">
      <section class="pro-card">
        ...
        <form class="rf-editor-form" @submit.prevent="save">
          ...
          <div class="rf-form-actions"><nut-button type="primary" native-type="submit" :loading="saving">保存公告</nut-button><button type="button" class="rf-secondary-button" @click="reset">清空</button></div>
        </form>
      </section>
