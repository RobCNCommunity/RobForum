  ...
  <PageContainer title="头像框管理">
    ...
    <section v-else class="rf-admin-frame-list">
      <article v-for="item in items" :key="item.id" :class="{ disabled: !item.enabled }">
        ...
        <footer><button type="button" class="rf-secondary-button rf-button-small" @click="edit(item)">编辑</button><button v-if="item.enabled" type="button" class="rf-danger-button rf-button-small" @click="disable(item)">停用</button></footer>
      </article>
    </section>
  </PageContainer>
</template>
