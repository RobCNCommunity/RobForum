...
const selectedKey = computed(() => { const p = route.path; if (p.startsWith('/admin/')) return p.startsWith('/admin/settings')?'/admin/settings':p.startsWith('/admin/ads')?'/admin/ads':p.startsWith('/admin/notices')?'/admin/notices':p.startsWith('/admin/resources')?'/admin/resources':p.startsWith('/admin/payouts')?'/admin/payouts':p.startsWith('/admin/wallet')?'/admin/wallet':p.startsWith('/admin/verifications')?'/admin/verifications':p.startsWith('/admin/users')?'/admin/users':'/admin/posts'; if (p.startsWith('/settings')) return '/settings/profile'; if (p.startsWith('/verification')) return '/verification/apply'; if (p.startsWith('/membership')) return '/membership'; if (p.startsWith('/check-in')) return '/check-in'; if (p.startsWith('/resources')) return '/resources'; if (p.startsWith('/wallet')) return '/wallet'; if (p.startsWith('/messages')) return '/messages'; if (p.startsWith('/notifications')) return '/notifications'; if (p.startsWith('/bookmarks')) return '/bookmarks'; if (p.startsWith('/music')) return '/music'; if (p.startsWith('/lottery')) return '/lottery'; if (p.startsWith('/users')) return auth.user && p === `/users/${auth.user.id}` ? p : '/users'; if (p.startsWith('/boards/')) return p; return '/' })
