...
const avatarFrameColumns = `id, name, description, style, primary_color, secondary_color, price_cents, allowed_regular, allowed_member, allowed_admin, enabled, sort_order, sales_count, created_at, updated_at`
...
func scanAvatarFrame(scanner rowScanner) (domain.AvatarFrame, error) {
	...
	err := scanner.Scan(
		&item.ID, &item.Name, &item.Description, &item.Style, &item.PrimaryColor, &item.SecondaryColor,
...
func equippedAvatarFrames(queryer sqlQueryer, userIDs []int64) (map[int64]*domain.AvatarFrame, error) {
	...
	for rows.Next() {
		...
		var role string
		if err := rows.Scan(
			&userID, &item.ID, &item.Name, &item.Description, &item.Style, &item.PrimaryColor, &item.SecondaryColor,
			&item.PriceCents, &allowedRegular, &allowedMember, &allowedAdmin, &enabled, &item.SortOrder,
